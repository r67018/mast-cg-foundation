...
def display():
    ...
    glPushMatrix()
    glColor3d(1., 0., 0.)
    glRotated(centerRotateAngle, 0, 0, 1)
    glTranslated(0.5, 0, 0)
    glRotated(rotateAngle, 0, 0, 1)
    glCallList(ID_DRAW_STAR)
    # 黒い星
    glColor3d(0., 0., 0.)
    glRotated(blackstarRotateAngle, 0, 0, 1)
    glTranslated(0.5, 0, 0)
    glScaled(0.1, 0.1, 1)
    glCallList(ID_DRAW_STAR)
    glPopMatrix()
...
#一定時間ごとに呼び出される関数
def timer(value):
    global rotateAngle, centerRotateAngle, blackstarRotateAngle
    rotateAngle+=1 # 回転角度の更新
    centerRotateAngle += 3
    blackstarRotateAngle += 10
